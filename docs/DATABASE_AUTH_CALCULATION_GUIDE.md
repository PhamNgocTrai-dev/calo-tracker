...
3. Có Supabase env, signing secret, Supabase identity và signed deadline chưa hết 5 phút: đọc/ghi PostgreSQL theo tài khoản hiện tại.
...
```json
{
  ...
  "database": "configured",
  "absoluteSession": "configured"
}
```

Nếu `database` là `unconfigured`, ứng dụng chưa đọc được Supabase env hợp lệ. Nếu `absoluteSession` là `unconfigured`, còn thiếu signing secret cho phiên 5 phút. Cả hai trường hợp đều fail closed với các trang dữ liệu.
...
Migration catalog bổ sung loại thực phẩm, nhóm, image key và image snapshot. Hai migration Water Tracker tạo từng lần uống nước với index/RLS rồi mở rộng `amount_ml` thành số nguyên 50–2.000 ml. Seed có thể chạy lại để cập nhật hơn 50 món, nguyên liệu và gia vị mà không tạo bản ghi verified trùng.
...
Tạo secret bằng `node -e "console.log(require('node:crypto').randomBytes(48).toString('base64url'))"`, đặt trực tiếp trong `.env.local`, không gửi/commit và không dùng tiền tố `NEXT_PUBLIC_`. Thay URL và publishable key bằng giá trị thật, sau đó khởi động lại server:
...
4. Supabase SSR lưu identity bằng auth cookies; server đồng thời tạo cookie deadline HMAC ràng buộc user ID và hết tuyệt đối sau 5 phút.
5. `proxy.ts` kiểm tra deadline trước và sau khi đọc/refresh Supabase claims; refresh, reload, chuyển trang và thao tác không gia hạn deadline.
6. Server gọi `auth.getUser()`, kiểm tra lại deadline bằng server clock và yêu cầu user ID khớp cookie đã ký.
7. `lib/auth/session.ts` trả về user cùng `expiresAtMs`/`serverNowMs` cho header; Client Component chỉ dùng hai timestamp này để hiển thị countdown.
8. Khi còn 1 phút, UI cảnh báo; tại 0 tự POST `/auth/logout`. Tab bị pause vẫn mất quyền server đúng deadline và bị chuyển login ngay khi hoạt động lại.
9. Profile sức khỏe được đọc từ `public.profiles` theo user ID hiện tại và RLS.

Không lấy user ID, login time hoặc expiry từ browser. Client countdown không authoritative. Không sao chép access token, refresh token hoặc custom session key vào Local Storage/Session Storage; `caloflow-theme` trong localStorage chỉ là preference giao diện không nhạy cảm.
...
| `public.water_entries` | Từng lần uống nước, lượng ml và thời điểm do PostgreSQL ghi                               |
...
BMI được dẫn xuất từ chiều cao và cân nặng đã qua validation:

```text
BMI = cân nặng kg / (chiều cao mét × chiều cao mét)
```

Ứng dụng hiển thị một chữ số thập phân và phân loại theo chính giá trị đã làm tròn:
...
BMI không được lưu thành cột riêng; calculator tính từ input đã validate, dashboard tính lại từ `profiles.height_cm` và `profiles.weight_kg`. Browser không được gửi BMI authoritative. BMI là chỉ số sàng lọc cho người trưởng thành, không đo trực tiếp thành phần cơ thể và không phải chẩn đoán.
...
### 8.2 Điều chỉnh theo thời gian yêu cầu
...
### 8.3 Giới hạn an toàn của ứng dụng
...
### 8.4 Không đề xuất thấp hơn BMR
...
### 8.5 Tốc độ thay đổi được đề xuất
...
### 8.6 Thời gian ước tính sau khi áp giới hạn
...
## 11. Quy tắc Water Tracker

Browser gửi một trong ba preset `250`, `350`, `500` ml hoặc lượng tùy chỉnh. Server Action chỉ chấp nhận số nguyên từ 50 đến 2.000 ml, xác thực user và tự gán `user_id`; PostgreSQL áp dụng cùng constraint và tự tạo `drank_at`. Browser không được quyết định ngày, timestamp, tổng nước hoặc user ID.

Mỗi lần uống là một row trong `water_entries`. Dashboard lấy các row gần hiện tại, lọc theo ngày tại timezone trong `profiles` và cộng `amount_ml` trên server. Timezone thiếu hoặc không hợp lệ fallback về `Asia/Ho_Chi_Minh` để dashboard không bị lỗi.

Mục tiêu MVP cố định:

```text
daily water target = 2000 ml
progress percent = min(100, total_ml / 2000 × 100)
remaining_ml = max(0, 2000 - total_ml)
```

Tổng thật vẫn có thể vượt 2.000 ml; chỉ chiều dài progress bị giới hạn 100%. Giới hạn 2.000 ml của validation áp dụng cho một entry, không phải tổng ngày. RLS cho phép mỗi tài khoản chỉ đọc, thêm và xóa row của chính mình; không có update policy.

## 12. Quy tắc lưu mục tiêu
...
## 13. Kiểm tra DB thật end-to-end
...
4. Đăng nhập.
5. Thêm món tại `/meals`, reload và kiểm tra row trong `meal_entries`.
6. Tại dashboard, thêm nhanh 250 ml và nhập tùy chỉnh 750 ml; reload và xác nhận tổng 1.000 ml.
7. Thử 49 ml, 2.001 ml và số thập phân để xác nhận bị từ chối; xóa lần 250 ml và xác nhận tổng còn 750 ml.
8. Tính và lưu goal tại `/calculator`; kiểm tra BMI đúng công thức và cùng kết quả trên dashboard.
9. Kiểm tra `profiles` và `goals`; xác nhận không có cột BMI dẫn xuất.
10. Quay lại `/`, dashboard hiển thị dữ liệu PostgreSQL.
11. Đăng nhập và xác nhận countdown gần 05:00; reload/chuyển trang không reset, warning ở 01:00 và automatic logout tại 00:00.
12. Để tab background quá 5 phút rồi quay lại; server phải từ chối ngay. Thay đổi system clock hoặc DOM countdown không được kéo dài quyền.
13. Kiểm tra Local Storage/Session Storage không chứa Supabase access token, refresh token hoặc custom auth key; signed deadline cookie phải `HttpOnly`.
14. Thử bằng hai tài khoản để xác nhận cookie không dùng chéo được và RLS không cho đọc/xóa chéo meal/water entries.

...
Water Tracker có ba nút nhanh 250/350/500 ml và ô nhập tùy chỉnh. Mỗi lần nhập tùy chỉnh phải là số nguyên từ 50 đến 2.000 ml. Mục tiêu hiện tại cố định 2.000 ml/ngày:

```text
Tiến độ = min(100%, tổng ml hôm nay / 2.000 × 100%)
Còn lại = max(0, 2.000 - tổng ml hôm nay)
```

Tổng hiển thị vẫn có thể vượt 2.000 ml; chỉ thanh tiến độ dừng ở 100%. Mỗi lần uống là một bản ghi riêng để có thể xóa đúng lần nhập nhầm.

## 3. BMI là gì?

BMI (Body Mass Index) là chỉ số sàng lọc dựa trên chiều cao và cân nặng:

```text
BMI = cân nặng (kg) / bình phương chiều cao (m)
```

CaloFlow hiển thị BMI với một chữ số thập phân và dùng các nhóm tham khảo cho người trưởng thành:
...
Ví dụ người nặng 70kg và cao 1,75m:

```text
BMI = 70 / (1.75 × 1.75) ≈ 22.9
```

BMI không đo trực tiếp tỷ lệ mỡ, khối lượng cơ, phân bố mỡ hoặc sức khỏe chuyển hóa. Vận động viên, người mang thai, người cao tuổi và một số nhóm khác có thể cần cách đánh giá phù hợp hơn.
...
## 6. Mục tiêu calo được tính như thế nào?
...
## 7. Các giới hạn thận trọng của ứng dụng
...
## 8. Tốc độ và thời gian ước tính
...
- server tự tính lại BMI, BMR, TDEE và mục tiêu calo; BMI được dẫn xuất, không lưu thành cột riêng;
- lượng nước được kiểm tra lại trên server và bằng constraint PostgreSQL;
- Supabase SSR xác minh identity và cookie deadline do server ký giới hạn mỗi lần đăng nhập đúng 5 phút; countdown chỉ hiển thị, không có quyền gia hạn;
- không lưu auth token, signing secret hoặc custom key trong Local Storage/Session Storage;
- mỗi tài khoản chỉ được đọc, thêm hoặc xóa dữ liệu của chính mình nhờ xác thực và Row Level Security.

## 10. Cách dùng kết quả hợp lý
